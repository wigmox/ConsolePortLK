...
function ConsolePort:CalibrateController(reset)
	...
	if not self.calibrationFrame then
		...
		cbF.HelpButton:SetNormalTexture("Interface\\AddOns\\ConsolePort\\Textures\\Interface\\help-i")
		cbF.HelpButton:SetHighlightTexture("Interface\\AddOns\\ConsolePort\\Textures\\Interface\\help-i")
		...
		do  -- Regions
			...
			cbF.Binding 	= cbF.Container:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
			...
			local function ProcessStickInput(self, input)
				...
				if stickRadialType then
					...
					if customDetected then
						...
						CPAPI.TimerAfter(1, function() self:Hide() end)
					end
				else
					NoneDetected(self)
				end
		...
		cbF:SetScript("OnShow", function(self)
			...
			CPAPI.SetShown(self.Container, not self.StickInput:IsShown())
		end)
