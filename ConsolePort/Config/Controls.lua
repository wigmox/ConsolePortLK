...
function WindowMixin:Save()
	...
	db('nameplateShowEnemies', scrape)
...
db.PANELS[#db.PANELS + 1] = {name = 'Controls', header = SETTINGS, mixin = WindowMixin, onCreate = function(Controls, self)
		...
		function IBFullModule:OnShow()
			...
			if self.Enable:GetChecked() then
				...
				if binding then
					...
					if id then 
						local mainType, _ , subType, id = GetActionInfo(id) 
	...
	do 	local count = 0
		...
		Controls.TopLine:SetTexture('Interface\\AddOns\\ConsolePort\\Textures\\Lines')
		...
		Controls.BottomLine:SetTexture('Interface\\AddOns\\ConsolePort\\Textures\\Lines')
